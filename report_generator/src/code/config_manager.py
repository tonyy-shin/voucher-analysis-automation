...
_DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "display_labels": {
        ...
        # 섹션4 분류 근거 — 고정 항목 / 동적 항목 두 갈래.
        #   rows(고정 항목) : csv_column을 지정하고, 출력전표와 무관하게 출력.csv 전체에서
        #       그 컬럼의 non-empty 셀을 순서대로 수집해 렌더한다(모든 보고서 동일). 참조는
        #       조회 중인 출력전표 행 기준. 값이 비어도 라벨 행은 항상 표시된다.
        #   동적_컬럼_목록(동적 항목) : 조회 중인 출력전표 행에 값이 있을 때만 그 보고서에
        #       label=컬럼명으로 새 항목이 생성된다(값 없으면 항목 자체가 없음).
        #   셀 값은 이미 "라벨: 내용" 완성형이며 빈 셀은 건너뛴다.
        # 근거_컬럼_목록 : 문구 설정 탭의 고정행 csv_column 드롭다운 추천 후보 (자유 입력도 허용).
        # rows/동적_컬럼_목록 은 "문구 설정" 탭에서 편집되며, 삭제 시 PDF에서도 제거된다.
        "section4": {
            "제목": "4. 분류 근거",
            "근거_컬럼_목록": ["직공통비", "성격별분류"],
            "rows": [
