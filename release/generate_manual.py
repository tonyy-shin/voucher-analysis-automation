...
def build_story(s: dict) -> list:
    ...
    story.extend(bullets([
        ...
        "<b>성격별 분류(6종)</b> — 계약비·유지비·손해조사비·투자관리비·간접비·공통비를 부점별로 교차 집계합니다.",
    ...
    req_rows = [
        [Paragraph("사업비정보.csv", s["td_b"]),
         ...
         Paragraph("계약비·유지비·손해조사비·투자관리비·간접비·공통비·합계", s["td"])],
    ...
    faqs = [
        ...
        ("Q5. 성격별 금액이 0으로 나와요",
         "① 사업비정보.csv의 성격 컬럼(계약비·유지비·손해조사비·투자관리비·간접비·공통비) 값이 "
    ...
    hist_rows = [
        [Paragraph("v2.0.0", s["td_b"]), Paragraph("2026/06", s["td_c"]),
         Paragraph("입력 형식을 엑셀(다중 시트)에서 <b>CSV 4개 파일</b>로 전환; 금액 소스를 "
                   "'합계' 컬럼으로 정리; 성격 분류 컬럼을 <b>6종</b>(계약비·유지비·손해조사비·"
